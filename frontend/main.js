...
async function loadDashboard() {
  ...
  const totalTxns = (monthly.items || []).reduce((s, r) => s + (r.txn_count || 0), 0);
  const totalInTxns = (monthly.items || []).reduce((s, r) => s + (r.txn_in_count || 0), 0);
  const totalOutTxns = (monthly.items || []).reduce((s, r) => s + (r.txn_out_count || 0), 0);
  el('kpi-tx').textContent = `${totalTxns} (${totalInTxns} nhập, ${totalOutTxns} xuất)`;
...
// Product CRUD modal handlers with suppliers
...
async function openProdCrudModal(data) {
  ...
  // Tab 2: Populate warehouse dropdown
